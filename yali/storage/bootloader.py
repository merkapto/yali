...
class BootLoader(object):
    ...
    def writeDeviceMap(self, usedDevices):
        with open(os.path.join(ctx.consts.target_dir, "boot/grub2", self._deviceMap), "w+") as deviceMapFile:
        ...
        yali.util.cp(os.path.join(ctx.consts.target_dir, "boot/grub2", self._deviceMap), "/tmp/device.map")
...
""" % (bootPartitionPath, stage1Path)
    ...
    def install2(self):
        ...
        if yali.util.isEfi():
            ...
            yali.util.chroot("mount -t efivarfs efivarfs /sys/firmware/efi/efivars")
            ...
            # postinstall.py içinde writeBootlooder()'a eklenmeli
